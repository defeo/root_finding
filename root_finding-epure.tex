...
\comd
A mon avis ce paragraphe doit \^etre merg\'e avec la suite car ici on est dans background and notations.
\comf
...
% the computation of the minimal polynomial of those intersections by means of gcd's, i.e. 
 \begin{equation}
 \label{base_ARM}
\gcd(f(X),L_i(X)-\ell_i)     \mbox{ with }  \ell_i  \in V_i^\ast\,,
\end{equation}
...
\section{Expected number of iterations of ARM}
...
Consider the $\qq$-ary tree were each node may be uniquely specified by $\pi_i(\rho)$ and contains $f_{n-i,\rho}$ the minimal polynomial of the distinct roots of $f$ belonging to $V_{n-i,\rho}$. The root of the tree is specified by $\emptyset$ and contains $f_{n,0}(X)=\gcd(f(X),X^{\qq^\nn} -X)$. If each branch from a node are labelled with a element of $\basef$, the components of $\pi_i(\rho)$ specify the path from the root to that node.
...
\noindent {\bf Link between digital tries and our context:} The main difference between the two contexts is that the height of our tree is bounded by the degree of the field extension, denoted by $H'_\dd$, while the height of digital tries $H_\dd$ are only bounded by the number of keys they contain. If the roots of the root of the tree $f_{n,0}$ are uniformly chosen in $\extf$ and independently and assumed to be $\dd$, in this case the key are the roots of $f_{n,0}$ and it is not hard to see that $H'_\dd \le min(H_\dd,\nn)$. 
 Note that in practice, the roots of $f_{n,0}(X)=\gcd(f(X),X^{\qq^\nn} -X)$ are not chosen independently because the $\gcd$ guarantees that all the roots are different.
...
\comd
J'ai un problème je ne sais pas si je dois parler de $f$ ou $f_{n,0}(X)=\gcd(f(X),X^{\qq^\nn} -X)$. Ce qui importe pour le comportement de l'arbre c'est $f_{n,0}(X)$.
\comf 
