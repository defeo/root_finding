...
$$f(x)=0.$$
%
This problem has a lot of applications, in particular in cryptography and in coding theory. It also has a rich history, with many strategies proposed over the years.
...
E.R. Berlekamp deduces the factorization $f(X)=\prod_{\alpha \in \mathbb{F}_{\qq}} \gcd(f(X),h(X)-\alpha)$. The method %is deterministic and it 
...
about the finite affine geometry $\AG$. In Sec 3 we present two (three?)
root finding algorithms based on the structure of $\AG$. In Sec 4 we
present some refinements on the implementation of those algorithms. In Sec 5
we present experimental results. Finally we list some open problems
and future directions in Sec 6 (do we?).
...
%  \underset{n \to +\infty}{\overset{w^*}{\longrightarrow}}0
 ...
 bottom of the tree. The goal of Section~\ref{sec:avg} is to analyze
 the behavior of ARM and SRA on average.
...
f(X)=\prod_{r\in\fq}\gcd\bigl(\Tr(\alpha X)-r,f(X)\bigr)
\end{equation}
...
% 
On average, $\bigO(\log(\nn))$ attempts are required to find one such element \cite{Menvanovans92}. 

The polynomial $\Tr(\alpha X) \mod f$ is computed iteratively using
$O(\nn\Mul(d)\log\qq)$ operations. If the result is not a constant,
the $\qq$ gcds are computed in $O(\qq\Mul(d)\log\dd)$, otherwise the
algorithm must be restarted with a different $\alpha$.  Therefore,
splitting $f$ in at least two distinct factors costs
$\bigO(\nn\log\nn\Mul(\dd)\log\qq + \qq\Mul(\dd)\log\dd)$ on average,
or $O(\nn^2\Mul(\dd)\log\qq+\qq\Mul(\dd)\log\dd)$ in the worts case.

The process is then recursively applied on the factors. It is possible
to prove that the full decomposition of $f$ into its linear factors is
attained after $O(\log\dd)$ iterations on average
(see~\cite{Gathen2003}, Th 14.11). In the worst case, it will require
$\bigO(n^2 \cdot \log(n) \cdot q \cdot \log(q) \cdot M(d) )$
operations.
%\end{proof}
...
$\upsilon_\nn=\alpha^{-1}$, then we exactly have
$L_{\nn-1}(X)=\alpha^{-q^{n-1}}\Tr(\alpha X)$ and
\begin{equation*}
  M_{\nn-1,r}(X)=
  \alpha^{-\qq^{\nn-1}}\Tr(\alpha(X-r/\alpha))=
  \alpha^{-\qq^{\nn-1}}\left(\Tr(\alpha X)-r\right).
\end{equation*}
Up to the constant factors $\alpha^{-q^{n-1}}$, the polynomials used
in BTA and in the last level of ARM are therefore identical.

From a geometric point of view, each trace computed by BTA separates the roots of $f$ into $q$ different afine sets of size $\qq^{\nn-1}$, corresponding to the roots of $\Tr(\alpha X)-r$ for all $r\in\fq$. After repeating this separation process for $O(\log_\qq\dd)$  randomly chosen $\alpha$ values, the intersections of these afine sets are likely to become singletons, each singleton containing one root of $f$. 
%
...
\noindent {\bf Average case running time of SRA:} According to ARM algorithm, the number of iterations $h_d$ is the smallest value for which $\deg f^{(h)}=\qq^{n-h}$. Its means that $h_d$ is the 
 ...
 $H'_d/\log_{q} d \underset{d \to +\infty}{\longrightarrow} 1$ a.s. It follows that $h_d \approx n-\log_{q} d$.
...
\noindent {\bf Variant with partial precomputation\label{sec:BTA:ARMvariant}:} The precomputation step of ARM and SRA costs $O(\nn^2 \log \qq)$ to determine the $\alpha_i$'s involved in the decomposition the field equation.
...
Note that step~\ref{alg:arm:Li} of algorithm~\ref{alg:arm} which consists in computing $L_i \bmod f$ need to be slightly modified. The values $X^{q^j}\bmod f$ are first computed in $O(M(d)n\log q)$ operations, then $L_i\bmod f$ can be computed in $O(\dd \nn)$ operations. This does not change the complexity of the step. Step \ref{alg:arm:combili} need also to be modified because we don't have matrix $\Gamma$ defined in \eqref{eq:Gamma}. Instead of computing in step \ref{alg:arm:combili}
 the values $\ell_{i-1, \rho}$ for all the nodes of the tree, we compute the values $\ell_{i-1, \rho}= L_{i-1}(\rho)$ by simple evaluation of the polynomial $L_{i-1}$ for any any root approximation $\rho=\sum_j r_j v_j$ candidate appearing in step~\ref{alg:arm:gcd}. Each value $\ell_{i-1, \rho}$ requires $\bigO(\nn \log \qq)$ operations and there are at most $d \cdot h$ such value to compute. The overall complexity for the computation of these values is $O(h \nn\dd  \log \qq )$ which is not a bottleneck in the average case. 
