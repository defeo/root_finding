...
\noindent {\bf ARM}: The method consists in \emph{projecting} the roots of $f$ onto the spaces $V_i^\ast$ while simultaneously \emph{intersecting} those with the corresponding fibers of $L_i$ identified by $l_i$.
...
%Note that the $l_i$ for which $\gcd(f(X),L_i(X)-l_i) \ne 1$ correspond to the $L_i$-projections onto $V_i^\ast$ of the roots of $f$.
 ...
 \begin{algorithm}
   ...
   \FOR {$\nn \ge i \ge 1$}
   \FOR {any node $f_{k,r}$ of degree $>1$ in the tree}
   \STATE Compute $\ell_{i-1,r} = L_{i-1}(r) = \sum_j r_j\gamma_{i-1,j}$;
   \ENDFOR
   \FOR {any leaf in the tree $f_{i,r}$ of degree $>1$}
   \STATE Compute $\bar{L}_r = L_{i-1}\mod f_{i,r}$;
   \FOR {any $c\in\basef$}
   \STATE $f_{i-1,r+c\cdot v_i} = \gcd(f_{i,r}, \bar{L}_r - \ell_{i-1,r} - c\gamma_{i-1,i})$;
   \ENDFOR
   \ENDFOR
 ...
 \begin{Lem}
   ...
   input $f$ and $v_1,\dots,v_\nn$ has depth $h$, it computes its
   output using $O(n\Mul(d)\log q + h^2d + h\qq\Mul(d)\log d)$ operations over $\extf$.
 \end{Lem}
 \begin{proof}
   \todo{Breakup:
     \begin{itemize}
     \item Step 3: $n\Mul(d)\log q$,
     \item Step 7: $h^2d$ (even $hd$ on average),
     \item Step 9: $h\Mul(d)\log d$ by a subproduct tree,
     \item Step 11: $h\qq\Mul(d)\log d$.
     \end{itemize}
   }
 \end{proof}
