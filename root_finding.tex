...
Then we compute all $K_{1,i_1}$ and $K_{2,i_2}$ needed modulo $f$ by linear combinations, at a total cost of $O(d(\min(S,n_1)n_1+\max(1,S/n_1)n_2))$ operations over $\mathbb{F}_{\qn}$. Finally, we compute the $S$ compositions of $K_{1,i_1}$ and $K_{2,i_2}$ modulo $f$ needed. 
%
...
We finally show how to compute $L_{i_1,i_2}(\rho)=L_{i_1,i_2}(X)\bmod(X-\rho)$. 
We use the same technique as the one we used to compute $L_{i_1,i_2}\bmod f$, but replacing the polynomial $f(X)$ by $f'(X):=X^m-\rho^m$ with $m\in\{2,3\}$ such that $\gcd(m,q)=1$. This produces $L_{i_1,i_2}\bmod f'$, which we reduce modulo $X-\rho$ to obtain $L_{i_1,i_2}(\rho)$.
