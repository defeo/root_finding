...
\title{Applications of the affine geometry of {\LARGE$GF(\qq^\nn)$} to root finding}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \STATE\label{alg:resultant:select} Select points $\delta_0,\dots,\delta_\dd$ s.t. $\beta\not\in\Span(\delta_0,\dots,\delta_\dd)$;
...
\begin{proof}
 ...
 % To make things simpler, we start by changing the sign of the right
  ...
  %\end{equation}
  Let now $\Delta_i=\delta_i^\qq-\beta^{\qq-1}\delta_i$. By linearity
  we have
  \begin{multline}
    \label{eq:lin-resultant}
    \Res_X\bigl(X^\qq-\beta^{\qq-1}X-\Delta_i,f(X)\bigr) =\\
    \Res_X\Bigl(  \prod_{c\in\extf}(X-\delta_i - c\beta),f(X) \Bigr) =\\
    \prod_{c\in\extf} \Res_X\bigl(X-\delta_i - c\beta, f(X)\bigr) =\\
    (-1)^{\qq\dd} \prod_{c\in\extf} f(\delta_i+c\beta),
  \end{multline}
  where the second and third equalities come from the multiplicativity
  of the resultant. Observe that $\Delta_0,\dots,\Delta_d$ are all
  distinct, otherwise one would have
  $\beta^{\qq-1}=(\delta_i-\delta_j)^{\qq-1}$ for some $i$ and $j$,
  contraddicting $\beta\not\in\Span(\delta_0,\dots\delta_\dd)$.  The
  correctness of the algorithm then follows immediately.

  We now go to the complexity. To construct the $\delta_i$'s we simply
  enumerate vectors in a span not containing $\beta$. This is possible
  as long as $\dd<\qq^{n-1}$, and costs $d$ operations in
  $\extf$. Step~\ref{alg:resultant:multi-ev} is computed using
...
If $\dd\ge\qq^{\nn-1}$, we can't find enough $\delta_i$'s for
step~\ref{alg:resultant:select} of
Algorithm~\ref{alg:resultant:select}. In this case,
Eq.~\eqref{eq:lin-resultant} cannot be applied anymore, however the
evaluation-interpolation approach is still possible at a slightly
worse cost. Note that this is not a concern for root finding, as
exhaustive search is more efficient in this case.
...
From a geometric point of view, each trace computed by BTA separates the roots of $f$ into $q$ different affine sets of size $\qq^{\nn-1}$, corresponding to the roots of $\Tr(\alpha X)-r$ for all $r\in\fq$. After repeating this separation process on the factors for $O(\log_\qq\dd)$ randomly chosen $\alpha$ which are linearly independent, the intersections of these affine sets are likely to contain one root of $f$, leading to its linear factors.
...
ARM may perform poorly in the worst case. When the number of iterations $h$ is maximal, 
the term corresponding to the computation of the $\ell_{i,\rho}$'s in step \ref{alg:arm:combili} becomes $\bigO(\nn^2 \dd)$, which may become the bottleneck of the algorithm. 
Note that the same computation is also performed in SRA.

If a normal basis with fast arithmetic is available for $\extf/\basef$ (e.g., using the generic construction of~\cite{Couveignes-Lercier}), the  $\ell_{i,\rho}$'s  may be computed more efficiently.

At every iteration, for each node $f_{i+1,\rho}$ that needs to be branched, the values $\ell_{i,\rho}$ corresponding to its $\qq$ children may be computed from $\ell_{i+1,\rho}=L_{i+1}(\rho)$ by solving the equation  
...
it follows that the set of $\ell_{i,\rho}$'s corresponding to the $\qq$ children of the node $f_{i+1,\rho}$ is given by 
$$\{ \beta_i \cdot \ell \mid \ell^q-\ell=\beta_i^{-q}\ell_{i+1,\rho} \} \,.$$
In ARM and SRA, we only need to determine one such element. We do this by first computing the right side of the equation in $O(\log\qq)$ operations in $\extf$. If the field extension $\extf/\basef$ is represented by a normal basis, a particular solution of the equation may be derived by solving a bidiagonal system in $n$ variables belonging to $\basef$, which requires $\bigO(\nn)$ operations in $\basef$ or $O(1)$ operations in $\extf$.     Finally, the solution is multiplied by $\beta_i$ again at a cost $\bigO(1)$. It follows that for each node $f_{i+1,\rho}$, the computation of one child $\ell_{i,\rho}$ costs $\bigO(\log\qq)$ operations in $\extf$. As the number of leaves at every iteration is upper bounded by $\dd$, the overall cost for computing  all the $\ell_{i,\rho}$'s in step \ref{alg:arm:combili}  is $O(\dd\log\qq \log_\qq \dd)$ on average and $O(\nn\dd\log\qq)$ in the worst case, which is no more a bottleneck.
...
The precomputation step of ARM and SRA costs $O(\nn^2 \log \qq)$ to determine the $\alpha_i$'s involved in the decomposition of the field equation.

Performing on average $2\log_\qq d$ iterations, most of the time ARM only needs the last $2\log_\qq d$ values $\alpha_i$ to split $f$ completely.
In what follows, $h$ is the \emph{a priori} bound on the expected number of iterations of ARM, typically $c \cdot \log_\qq d$ with $c \ge 2$. 


As seen in Eq.~\eqref{dual_polynomial}, for any linearized polynomial $L_i$ there exists a unique linearized polynomial $L_i^\ast$ such that $(L_i^\ast\circ L_i)(X)=(L_i\circ L_i^\ast)(X)=X^{\qq^\nn}-X$. 
...
Therefore, we start  by computing $L_{n-h}^\ast(X)$ using a basis $\{\upsilon_{n-h+1},\ldots,\upsilon_n\}$ of a vector space of dimension $h$ of $\extf/\basef$. We deduce that
$$X^{\qq^\nn}-X=(X^{\qq}-\alpha_n) \circ \cdots \circ (X^{\qq}-\alpha_{n-h+1} X) \circ L_{n-h}(X)\,,$$
where $L_{n-h}(X)$ is unique and not yet computed. This requires $O(h^2\log\qq)$ operations in $\extf$. For i starting from $\nn$ and going down to $n-h$,
...
Depending on where the roots of $f$ are located, the a priori bound $h$ may in fact not be sufficient to completely factor $f$. In this case, we launch the algorithm on the the remaining factors of $f$ at level $h$ with a newly generated vector basis with $h'$ elements. This value should be chosen 
 as a multiple of $\log_{\qq} \max(\deg f_{h,\rho})$ where $f_{h,\rho}$ are the remaining factors of $f$ at level $h$.
...
decomposition so that the polynomial $L_{(m-1)s}$ is a
...
\end{equation*}
Thus $L_{(m-1)s}(X)=\Tr_{\ff{\QQ^m}/\ff{\QQ}}(X)$ is a projection
...
what was previously thought, e.g., for ARM. Thus we think that these
algorithms may find useful applications in particular finite field
problems.
