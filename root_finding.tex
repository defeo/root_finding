...
In ARM, the minimal polynomials of the $i$-flats $V_{i}$ are effectively computed and the $f_{i-1,\rho+c \cdot \upsilon_i}(X)$'s are computed as
 \begin{equation}
 \label{gcd_ARM}
 \gcd(L_{i-1}(X)-L_{i-1}(\rho + c \cdot \upsilon_i),f_{i,\rho}(X)) \,,
 \end{equation}
 where $L_{i-1}(\rho +  c \cdot \upsilon_i)$ is determined from precomputed values. ARM is summarized in Algorithm~\ref{alg:arm}.
...
\begin{proof}
  ...
  the roots of the $f_{i,\rho}$ for all $\rho$; this shows that $S$
  only contains roots of $f$. %
  By definition $\deg f_{0,\rho}\le 1$, thus $R_n$ is empty, and all
  the roots of $f$ eventually enter in $S$.
  ...
  $d$ of $f$, hence each $L_i$ is computed using in
  ...
  \]
  The linear combination on the right require at most $n^2$ products
  ...
  iteration only the first $h$ coefficients in the vector of $\rho$
  are non-zero. %
...
\begin{algorithmic}[2]
\REQUIRE {A polynomial $f\in\extf[X]$ of degree $\dd$,\\
A basis $\beta_1,\dots,\beta_\nn$ of $\extf$.}
\ENSURE {The roots of $f$ in $\extf$.}
\STATE Initialize tree with root $f_{\nn,0} = f \mod X^{q^n}-X$;
\FOR {$\nn \ge i \ge 1$}\label{alg:bta:for}
\FOR {any leaf in the tree $f_{i,\rho}$ of degree $>1$}
\FOR {any $c\in\basef$}
\STATE\label{alg:bta:gcd} $f_{i-1,\rho+c\cdot \upsilon_i} =\gcd( \Tr_{\extf/\basef}(\beta_i X) -c ,f_{i,\rho}(X) )$;
\ENDFOR
\ENDFOR
...
Steps~4-6 of Algorithm~\ref{alg:bta} use HOWMANY operations over $\fqn$.
