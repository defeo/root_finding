...
\noindent {\bf Average case running time of SRA:} According to ARM algorithm, for a fixed polynomial of degree $\dd$ the number of iterations $h_d$ is the smallest value for which $\deg f^{(h)}=\qq^{n-h}$. Its means that $h_d$ is the 
...
\noindent {\bf Average case running time of BTA:} This analysis follows from Menezes, Van Oorschot and Vanstone~\cite{Menvanovans92} and is also related to the statistical properties of digital tries. It can be show that the average complexity is $\bigO((\nn\Mul(d)\log\qq+\qq\Mul(d)\log\dd) \log\dd)$.
