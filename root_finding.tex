...
\begin{itemize}
\item \cite{Chien64a} Cyclic decoding procedures for Bose- Chaudhuri-Hocquenghem codes  -- R. T. Chien (exhaustive search for coding theory: a shortcut)
Brute force approach which consist in replacing all the multiplication between to random elements to multiplication by a constant.

\item \cite{berl67} Factoring Polynomials over Finite Fields -- E. R. Berlekamp
Linear alfgebra: Berlekamp Algebra.


\item \cite{berl70} Factoring Polynomials over Large Finite Fields -- E. R. Berlekamp

\item \cite{Moenck77} On the efficiency of algorithms for polynomial factoring -- Robert T. Moenck


\item \cite{Rabin79} Probabilistic Algorithms In Finite Fields -- Michael O. Rabin


\item \cite{cantor1981} A New Algorithm for Factoring Polynomials over Finite Fields -- D. Cantor and H. Zassenhaus

\item \cite{Ben-Or81}  Probabilistic Algorithms in Finite Fields -- M. Ben-Or

\item \cite{mBER84a} Least Affine Multiple -- E. R. Berlekamp (efficace si le degr\'e est petit)


%\item \cite{OorschotV90}   On splitting sets in block designs and finding roots of polynomials --  P. van Oorschot and S.A. Vanstone



\item \cite{MenezesOV88} On some computational aspects of root finding in $\extf$

\item \cite{OorschotV89}: A Geometric Approch to Root Finding in GF$(\qq^\nn)$ 

\item \cite{Shoup91b}  A Fast Deterministic Algorithm for Factoring Polynomials over Finite Fields of Small Characteristic: V. Shoup  (generalization de BTA mais deterministe+ idee de Legendre je crois a la fin)


\item \cite{Menvanovans92} Subgroup Refinement Algorithms for Root Finding in $GF(q)$ -- A.J. Menezes, P. van Oorschot and  S.A. Vanstone 


\item \cite{nied94} Factoring Polynomials over Finite Fields Using Differential Equations and Normal Bases -- H. Niederreiter

\item \cite{Fleis96} Comparative Implementations of Berlekamp's and Niederreiter's Polynomial Factorization Algorithms -- P. Fleischmann and P. Roelse


%\item \cite{Biswas09efficientroot}   Efficient root finding of polynomials over fields of characteristic 2 -- B. Biswas and V. Herbert



\item \cite{BKS13} Character Sums and Deterministic Polynomial Root Finding in Finite Fields -- J. Bourgain  and S. Konyagin and I. Shparlinski 


\item \cite{cgUCL-P14} Finding {R}oots in GF$(p^n)$ with the Successive Resultant Algorithm -- C. Petit
...
General:
\begin{itemize}
\item \cite{Gathen06} Who {W}as {W}ho in {P}olynomial {F}actorization -- J. von zur Gathen

\item \cite{Gathen2003} Modern Computer Algebra -- J. von zur Gathen
\end{itemize}
...
take $\Mul(m)\in\bigO(m \log m \log\log m)$.
...
Define the operator which associate to each element of $\extf$ its $k$ last components in the basis $\{\upsilon_1, \cdots, \upsilon_n\}$, i.e. 
$$\pi_k:\extf \rightarrow \mathbb{F}_{\qq}^{k}: \rho=\sum_j r_j \upsilon_j \mapsto \pi_k(\rho)=(r_{n-k+1}, \cdots , r_n)\,.$$
...
\section{Tuning of ARM and its link to BTA}
...
%Using this method to compute the $\ell_{i,\rho}$ values ensures that the second step of our algorithm requires at most $O(qnd\log \dd)$ multiplications in the worst case, comparable to the complexity of the first step.
%
...
\subsection{Variant with partial precomputation\label{sec:BTA:ARMvariant}}
...
By Berlekamp~\cite[Theorem..]{}, for all $i$ there exists a linearized polynomial $L_i^*$ such that $(L_i^\ast\circ L_i)(X)=(L_i\circ L_i^\ast)(X)=X^{\qq^\nn}-X$. Consequently, any set of SRA values $(\alpha_{1},\alpha_2\ldots,\alpha_{n})$ leads to the sets of valid values $(\alpha_{i},\alpha_{i+1}\ldots,\alpha_{n},\alpha_{1}, \ldots, \alpha_{i-1})$ for all $i$.
%
...
Given $\alpha_i$ and $L_{i+1}(X)=\sum_{j=0}^{i+1}a_jX^{q^j}$, we compute $L_i(X)=\sum_{j=0}^{i}b_jX^{q^j}$ in $O(\nn \log q)$ operations as follows: first we compute $\alpha_i^{-1}$, then we set $b_0=a_0\alpha_i^{-1}$, and we recursively set $b_j=(b_{j-1}^q-a_j)\alpha_i^{-1}$.
%
...
%
Finally given a candidate root approximation $r$ and a polynomial $L_i$, the value $L_i(r)$ is computed in $O(\nn \log \qq)$ operations by simple evaluation. 
%
