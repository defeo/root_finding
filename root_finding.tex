...
is related to the second one. The second algorithm, called the Nested
Subspaces Algorithm (NSA), is new, and similar in spirit to
Berlekamp's Trace Algorithm (BTA)~\cite{berl70}.
...
$\times$, $\div$) in $\basef$, and its space requirements in terms of
number of elements of $\basef$ stored. As customary, we use the
...
polynomials in $\basef[X]$ of degree at most $m$ can be multiplied in
$\Mul(m)$ operations ($+$, $\times$) in $\basef$, and we make the
usual super-linearity assumptions on
$\Mul$~\cite[Chapter~8]{Gathen2003}.  Using FFT multiplication, we can
take $\Mul(m)\in\bigOt(m)$.  Using the so-called \emph{Kronecker's
  trick}~\cite{todo}, bivariate polynomials in $\basef[X,Y]$ of degree
$(m,n)$ can be multiplied in $O(\Mul(mn))$.

\paragraph{Representation of $\extf$} We suppose that elements of
$\extf$ are represented via an $\basef$-basis, then elements of
$\extf$ can be added, subtracted or multiplied by a scalar
$c\in\basef$ using $O(\nn)$ operations. Throughout this document we
adopt the convention of using Greek letters to represent elements of
$\extf$, and Latin symbols for the elements of $\basef$.

In most of the document, we further suppose that elements are
represented on the monomial basis $1,\zeta,\dots,\zeta^{\nn-1}$, where
$\zeta$ is a generator of $\extf$ over $\basef$.  Using this
representation, elements of $\extf$ can be multiplied using
$O(\Mul(n))$ operations, and inverted using $O(\Mul(n)\log n)$
operations~\cite{todo}.  Using Kronecker's trick, polynomials of
degree at most $\dd$ in $\extf[X]$ can then be multiplied in
$O(\Mul(\dd\nn))$ operations. \todo{Check complexities for Euclidean
  division and gcd.}

In Section~\ref{sec:ext:variant}, we will need a normal basis,
instead. Using the \emph{elliptic normal bases}
of~\cite{Couveignes-Lercier}, we can multiply and invert elements of
$\extf$ for the same asymptotic cost as with monomial
bases. \todo{What about operations on $\extf[X]$?}
...
$L_0(\delta),\dots,L_{n-1}(\delta)$.  We finally define $V_i^\ast$ as
...
\begin{Lem}
  ...
  $O(\qq\nn^2\Mul(n))$ operations over $\basef$.
\end{Lem}
\begin{proof}
  \todo{This is just the usual precomputation algorithm.}
\end{proof}
...
On the other hand, NSA isolates the roots of $f$ by
...
r_j\upsilon_j$ of $f$, NSA computes successive approximations
...
By Berlekamp~\cite[Theorem..]{}, for all $i$ there exists a linearized polynomial $L_i^*$ such that $L_i^*\circ L_i=L_i\circ L_i^*=x^{q^n}-x$. Any set of SRA values $(\alpha_{1},\alpha_2\ldots,\alpha_{n})$ therefore leads to other sets of valid values $(\alpha_{i},\alpha_{i+1}\ldots,\alpha_{n},\alpha_1,\ldots,\alpha_{i-1})$ for all $i$.
...
We must now show how to compute $L_i\bmod f$ and $L_i(r)$ at no $O(n^2)$ precomputation cost. We now that $L_n(x)=x^{q^n}-x$. Moreover,
$$L_{i+1}(x)=L_i(x)^q-\alpha_iL_i(x).$$
Given $\alpha_i$ and $L_{i+1}(x)=\sum_{j=0}^{i+1}a_ix^{q^i}$, we compute $L_i(x)=\sum_{j=0}^{i}b_ix^{q^i}$ in $O(n)$ multiplications as follows: first we set $b_0=a_0\alpha_0^{-1}$, then we recursively set $b_i=(a_i-a_{i-1}^q)\alpha_i^{-1}$.
%
Given a candidate root approximation $r$ and a polynomial $L_i$, the value $L_i(r)$ is computed in $O(n)$ operations by simple evaluation. 
%
Finally, the polynomial $L_i\bmod f$ can be computed in $\tilde O(dn)$ operations, or $O(dn)$ operations after the values $x^{q^i}\bmod f$ have first been computed.

If the polynomial $f$ is not squarefree and split, it is more efficient to first perform a DDF using modular composition algorithms such as Kedlaya-Umans in theory, or ?? in practice. Starting from an a priori bound $S=2\log d$, we then compute the last $\alpha_i$ values and finally perform the second step of NSA as above.
%
When the a priori bound turns out to be insufficient (in the unlikely event that two roots belong to the same subspace $M_{i,r}$), this algorithm fails to completely factor $f$ into linear terms. However, the whole process (except for DDF) can then be launch again starting from a different basis on the smaller factors.
...
Note that values $\beta_i$ such that $\alpha_i=\beta_i^{p-1}$ can be computed simultaneously to the $\alpha_i$ at no further cost. As already pointed out above, we can write 
$$(x^Q-\alpha_i x)= \beta_i\circ(x^Q-x)\circ(\beta_i^{-1}).$$
%
...
$$L_{n_1i+j}=\circ_{\ell=1}^j(x^{q}-\alpha_\ell' x) \circ_{k=1}^i \left(\beta_k\circ(x^Q-x)\circ(\beta_k^{-1})\right)$$
