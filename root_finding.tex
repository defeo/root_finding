...
\section{Link between ASM and SRA}

In this section we show that ASM and SRA methods can be seen as
\emph{dual} to each other if we remember that
$$\gcd(f(X),g(X)) \ne 1 \mbox{  if and only if }\Res_X(f(X),g(X))=0\,.$$

\medskip

\noindent {\bf Pre-computation}: Both methods, ASM and SRA, start by computing the decomposition of the field equation following the procedure described in relation~(\ref{Li_generation}), i.e.
...
\noindent {\bf ASM}: The method consists in \emph{projecting} the roots of $f$ onto the spaces $V_i^\ast$ while simultaneously \emph{intersecting} those with the corresponding fibers of $L_i$ identified by $l_i$.
 It is achieved by the computation of the minimal polynomial of those intersections by means of gcd's, i.e. 
 \begin{equation}
 \label{base_ASM}
\gcd(f(X),L_i(X)-l_i)     \mbox{ for any }  l_i  \in V_i^\ast\,,
\end{equation}
for $i$ starting from $n$ and going down to 0.
It will anyway leads to the root of $f$ because 
$$\gcd(f(X),L_0(X)-l_0)=\gcd(f(X),X-l_0) \ne 1\,,$$ 
provide the linear factors of $f$. Practically, these gcd's are computed in a dichotomic way. Let us first define
 $$f_{i,r}(X)=\gcd(f(X),L_i(X)-L_i(r))\,.$$
where $r \in \extf$. Hence $f_{i,r}(X)=f_{i,r'}(X)$ if and only if $r-r' \in V_i$.
According to~(\ref{base_ASM}) and the fact that $V_n^\ast=\{0\}$, $f_{n,0}(X)$ is first computed, i.e.
 $$f_{n,0}(X)=\gcd(f(X),L_n(X))=\gcd(f(X),X^{q^\nn}-X)\,.$$
The recursive step is based on~(\ref{node_product}) and works as follow: for any $L_i(r) \in V_i^\ast$ such that $f_{i,r}(X)$ is neither a constant polynomial nor a linear factors,
  $$
  \begin{array}{lll}
  f_{i,r}(X)&=&\prod_{c \in \basef} \gcd(f_{i,r}(X),M_{i-1,r+c \cdot v_i}(X)) \\
               &=&\prod_{c \in \basef} \gcd(f_{i,r}(X),L_{i-1}(X)-L_{i-1}(r+c \cdot v_i)) \\
               &=&\prod_{c \in \basef} f_{i-1,r+c \cdot  v_i}(X)\\
  \end{array}              
  $$ 
 Note that according to the linearity of $L_i$, the value $L_{i-1}(r)$ is common to the $\qq$ polynomials $f_{i-1,r+c \cdot  v_i}(X)$ to be determined and must be computed only once. 
 ...
 linear factors of $f$ (without their algebraic multiplicity). Note that all the other gcd's of~(\ref{base_ASM}) can be easily deduced without any computation from the leafs of the tree but they won't bring any new information to our problem.
...
The recursive is based on the definition of $L_i$ (see relation~(\ref{Li_generation})) and the fact that the roots of $g^{(i)}$ are the projection onto 
  $V_i^\ast$ of the roots of $g^{(i-1)}$. It works as follow: for any root $l_i$ of $g^{(i)}$ belonging to $V_i^\ast$, check whether $g^{(i-1)}(l_{i-1})=0$
   for any $l_{i-1} \in V_i^\ast$ such that $l_{i-1}^q-\alpha_i \cdot l_{i-1}=l_i$. This procedure builds a tree where at level $i$ the nodes are the roots of $g^{(i)}$. 
...
\section{Improvements and Complexity analysis of ASM and SRA}
